...
{
    public int[] networkShape = { 4, 4, 4, 2 };
    public Layer[] layers;
    ...
    public class Layer
    {
        ...
        public void Forwards(float [] inputsArray)
        {
            nodeArray = new float [n_nodes];
            
            for (int i = 0; i < n_nodes; i++)
            {
                //sum of weight x inputs
    ...
    public float[] Brain(float [] inputs)
    {
        for (int i = 0; i < layers.Length; i++)
        {
            if (i == 0)
            {
                layers[i].Forwards(inputs);
                layers[i].Activation();
            }
            else if(i == layers.Length - 1)
            {
                layers[i].Forwards(layers[i - 1].nodeArray);
            }
            else
            {
                layers[i].Forwards(layers[i - 1].nodeArray);
